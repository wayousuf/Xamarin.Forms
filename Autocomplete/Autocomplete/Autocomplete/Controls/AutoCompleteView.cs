...
namespace Autocomplete.Controls
{
    public class AutoCompleteView : ContentView
    {
        ...
        public IEnumerable FilterSuggestions(IEnumerable suggestions, string keyword)
        {
            ...
            var searchWords = keyword.ToLower().Split(new string[] { " " }, StringSplitOptions.RemoveEmptyEntries);
            var result = suggestions.Cast<object>();
            foreach (var item in searchWords)
            {
                if (!string.IsNullOrEmpty(DisplayPropertyName))
                {
                    result = result.Where(x => x.GetType().GetRuntimeProperty(DisplayPropertyName).GetValue(x).ToString().ToLower().Contains(item)).ToList();
                }
                else
                {
                    result = result.Where(x => x.ToString().ToLower().Contains(item)).ToList();
                }
            }
